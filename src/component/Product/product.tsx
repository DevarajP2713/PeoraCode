...
import {
  ...
  Toast,
  classNames,
} from "../../common/primereactComponents";
import {
  CopyInitialState,
  InventoryListInitialState,
  SellingInitialState,
} from "../../models/InventoryList";
...
import {
  addItemToSPList,
  updateInventoryProductMultipleFields,
  updateItemToSPList,
  updateSingleFieldValueToSPList,
} from "../../services/SPServices";
import {
  LISTS,
  PRODUCT_FIELDS_DATA,
  PRODUCT_SELLING_PRICE_FIELDS_DATA,
  PRODUCT_STONE_DETAILS_FIELDS_DATA,
...
interface IMasterExportDatas {
  ...
  Size: string;
  Cutting: string;
  PcsPerJewelry: number;
  Quality: string;
  TotalStoneQty: number;
  VendorCode1: string;
}

const _objData: ISKUData = {
  ID: null,
  Title: "",
  Photos: "",
};

let _arrSku: ISKUData[] = [];
let _arrExportProduct: IExportProductData[] = [];
let _exportMasterArray: IMasterExportDatas[] = [];
...
const Product = () => {
  ...
  const [isUploadXLSXVisible, setIsUploadXLSXVisible] =
    useState<boolean>(false);
  const [isExportXLSXVisible, setIsExportXLSXVisible] =
    useState<boolean>(false);
  const [isVisible, setIsVisible] = useState<boolean>(false);
  const [arrSyncItems, setArrSyncItems] = useState<ISKUData[]>([]);
  const [arrNotSyncItems, setArrNotSyncItems] = useState<ISKUData[]>([]);
  const [isLoader, setIsLoader] = useState<boolean>(false);
  const [isOpenSkuNo, setIsOpenSkuNo] = useState<boolean>(false);

  let [
    selectedProductFieldsForImportExport,
    setSelectedProductFieldsForImportExport,
  ] = useState([PRODUCT_FIELDS_DATA[0]]);
  let [finalFieldsForImportExport, setFinalFieldsForImportExport] = useState([
    PRODUCT_FIELDS_DATA[0],
  ]);
  ...
  const getAllProducts = (): void => {
    ...
    productData = setOtherLookupData(productData, remainingLookUpData).then(
      (res: any) => {
        setProducts([...res]);
      }
    );
  ...
  const handleSubmit = async () => {
    setSubmitted(false);
    setBlocked(true);
    let isSKUExist = allInventoryData.find(
      (ele) =>
        ele?.Id !== state?.Id &&
        ele?.Title?.trim().toLowerCase() === state.Title?.trim().toLowerCase()
    );
    if (
      isSKUExist ||
  ...
  const handleSync = async () => {
    if (!state?.VendorRef1.trim().length) {
      ...
      setBlocked(true);
      const findVendorRef = allInventoryData.filter(
        (ele) => ele.VendorRef1 === state?.VendorRef1
      );
      for (let ele of findVendorRef) {
        ...
        await updateInventoryProductMultipleFields(
  ...
  const getChannelsSuggestedPrice = (
    chanelName,
    baseMetal,
    recoverableCost,
    totalCost
    ...
    if (baseMetal === "Gold") {
      priceData = allSellingPrice.filter(
        (x) => x.ChanelName === chanelName && x.BaseMetal?.Title === baseMetal
      )[0];
    } else {
      priceData = allSellingPrice.filter(
        (x) => x.ChanelName === chanelName && x.BaseMetal?.Title != "Gold"
      )[0];
    }

    if (chanelName === "Amazon" || chanelName === "Peora") {
      return Math.ceil(
        (totalCost * priceData?.MultiplyBy + recoverableCost) /
          priceData?.DivideBy
      );
    }
  ...
  const getTotalCost = (stoneData) => {
    ...
    const totalCost =
      totalStoneCost +
      (state.LDMountCost ? state.LDMountCost : 0) +
      (state.MiscCost1 ? state.MiscCost1 : 0) +
      (state.MiscCost2 ? state.MiscCost2 : 0);

    // if (totalCost) {
    setAmazonPrice({
      ...amazonPrice,
      Suggested: getChannelsSuggestedPrice(
        "Amazon",
        state.BaseMetal?.Title,
        parseInt(state.RecoverableCost?.Cost ? state.RecoverableCost?.Cost : 0),
        totalCost
      ),
    });
  ...
  const handleDropdownChange = (e) => {
    if (e.value) {
      if (e.target.name === "BaseMetal") {
        let insertType = [];
        const boxByBaseMetal = allBoxType.filter(
          (x) => x.BaseMetal?.Id === e.value.Id
        );
        const boxType = refactorAllBoxType.filter(
          (x) => x.Id === boxByBaseMetal[0].Id
        );

        if (state.Category && state.Category.CatName === "Rings") {
          if (e.value.Title === "Gold") {
            const insertTypeData = allInsertType.filter(
              (x) =>
                x.BaseMetal?.Title === "Gold" && x.Category?.CatName === "Rings"
            );
            if (insertTypeData.length > 0) {
              insertType = refactorAllInsertType.filter(
                (x) => x.Id === insertTypeData[0].Id
              );
            }
          } else {
            const insertTypeData = allInsertType.filter(
        ...
        setState({
          ...
          InsertType:
            insertType && insertType.length > 0 ? insertType[0] : null,
          InsertTypeId:
            insertType && insertType.length > 0 ? insertType[0].Id : undefined,
        });
        ...
        if (
          e.value.CatName === "Pendants" &&
          state.MetalType &&
          state.MetalType?.MetalTypeName
        ) {
          const chainTypeData = allChainType.filter(
            (x) =>
              x.Metal.filter(
                (y) => y.MetalTypeName === state.MetalType?.MetalTypeName
              ).length > 0 && x.Category?.CatName === "Pendants"
          );
          chainType = refactorAllChainType.filter(
            (x) => x.Id === chainTypeData[0]?.Id
          );
        }

        if (
          e.value.CatName === "Earrings" &&
          state.MetalType &&
          state.MetalType?.MetalTypeName
        ) {
          const backingTypeData = allBackingType.filter(
            (x) =>
              x.Metal.MetalTypeName === state.MetalType?.MetalTypeName &&
              x.Category?.CatName === "Earrings"
          );
          backingType = refactorAllBackingType.filter(
            (x) => x.Id === backingTypeData[0]?.Id
          );
        }

        if (e.value.CatName === "Pendants") {
          recoverableCost = refactorAllRecoverableCost.filter(
            (x) => x.Category?.CatName === "Pendants"
          );
        } else {
          recoverableCost = refactorAllRecoverableCost.filter(
            (x) => x.Category?.CatName != "Pendants"
        ...
        setState({
          ...
          ChainTypeId:
            chainType && chainType.length > 0 ? chainType[0].Id : undefined,
          BackingType:
            backingType && backingType.length > 0 ? backingType[0] : null,
          BackingTypeId:
            backingType && backingType.length > 0
              ? backingType[0].Id
              : undefined,
          RecoverableCost:
            recoverableCost && recoverableCost.length > 0
              ? recoverableCost[0]
              : null,
          RecoverableCostId:
            recoverableCost && recoverableCost.length > 0
              ? recoverableCost[0].Id
              : undefined,
        });
        ...
        if (
          state.Category &&
          state.Category.CatName === "Pendants" &&
          e.target.value.MetalTypeName
        ) {
          const chainTypeData = allChainType.filter(
            (x) =>
              x.Metal.filter(
                (y) => y.MetalTypeName === e.target.value.MetalTypeName
              ).length > 0 && x.Category?.CatName === "Pendants"
          );
          chainType = refactorAllChainType.filter(
            (x) => x.Id === chainTypeData[0]?.Id
        ...
        if (
          state.Category &&
          state.Category.CatName === "Earrings" &&
          e.target.value.MetalTypeName
        ) {
          const backingTypeData = allBackingType.filter(
            (x) =>
              x.Metal.MetalTypeName === e.target.value.MetalTypeName &&
              x.Category?.CatName === "Earrings"
          );
          backingType = refactorAllBackingType.filter(
            (x) => x.Id === backingTypeData[0]?.Id
    ...
    if (e.target.name === "BaseMetal" && e.value && state.TotalCost) {
      setAmazonPrice({
        ...
        Suggested: getChannelsSuggestedPrice(
          ...
          parseInt(state.RecoverableCost?.Cost),
          state.TotalCost
        ),
      });
    ...
    if (state?.Id) {
      updateSingleFieldValueToSPList(
        ...
        .then((res) => {
          ...
          const updateInventoryData = allInventoryData.map(
            ({ Inactive, ...inventoryEle }) => ({
              ...inventoryEle,
              Inactive:
                inventoryEle?.Id === state?.Id ? isStatusActive : Inactive,
            })
          );
          setAllInventoryData(updateInventoryData);
          if (isStatusActive) {
  ...
  const printFooterComponent = (
    <>
      <Button
        ...
        onClick={() => setIsPrintDialog(false)}
      />
      <Button
        label="Print"
        icon="pi pi-print"
    ...
    setTimeout(() => {
      ...
      if (!event.query.trim().length) {
        ...
        inputItems.forEach((element) => {
          let allFilteredData =
            _filteredSKUData && _filteredSKUData.length > 0
              ? _filteredSKUData
              : products;
          ...
          _filteredSKUData = allFilteredData?.filter((ele) => {
            return (
              (ele?.Title &&
                ele?.Title?.toLowerCase().includes(element?.toLowerCase())) ||
              (ele?.VendorRef1 &&
                ele?.VendorRef1?.toLowerCase().includes(element?.toLowerCase()))
            );
          });
        });
      }
  ...
  const skuTemplate = (option) => {
    return (
      <div className="col-12 flex align-items-center auto-complete-items">
        ...
        <div className="col-6">{option?.VendorRef1}</div>
      </div>
    );
  };
  ...
  const skuFooterTemplate = () => {
    return (
      <div className="col-12 flex align-items-center auto-complete-items-footer">
        <div className="col-6">SKU No.</div>
        <div className="col-6">Vendor Ref 1</div>
      </div>
    );
  };
  ...
  const addStone = async () => {
    ...
    if (_stone.StoneCode) {
      ...
      _stone["WrEaRdTotal"] = parseFloat(
        (_stone.WtEaRd * selectedStoneQuantity).toFixed(3)
      );
      _stone["StoneCost"] = parseFloat(
        (_stone.LanEa * selectedStoneQuantity).toFixed(2)
      );

      _stone["Sequence"] =
        selectedStonesData && selectedStonesData.length > 0
          ? selectedStonesData[selectedStonesData.length - 1].Sequence + 1
          : 1;
  ...
  const footerGroup = selectedStonesData && selectedStonesData.length > 0 && (
    <ColumnGroup>
      <Row>
        <Column className="text-right pr-2" footer="Total Ct Wt:" colSpan={5} />
        <Column
          className="text-right pr-2"
          footer={formatCurrency3(gridTotalValue("WrEaRdTotal"))}
        />
        <Column
          className="text-right pr-2"
          footer="Total Stone Cost:"
          colSpan={3}
        />
        <Column
          className="text-right pr-2"
          footer={formatCurrency(gridTotalValue("StoneCost"))}
        />
      </Row>
    </ColumnGroup>
  );
  ...
  const removeStone = (rowData) => {
    let selectedData = selectedStonesData.filter(
      (x) => x.Sequence !== rowData.Sequence
    );
      ...
      <>
        <InputNumber
          ...
          className={`w-full ${classNames({
            "p-invalid":
              !options.value ||
              (options.value && options.value > options?.rowData?.StQuantity),
          })}`}
          min={0}
  ...
  const onRowEditComplete = async (e) => {
    ...
    stoneItems[rowIndex].WrEaRdTotal = parseFloat(
      (newRowData.WtEaRd * value).toFixed(3)
    );
    stoneItems[rowIndex].StoneCost = parseFloat(
      (newRowData.LanEa * value).toFixed(2)
    );
  ...
  const stoneTemplate = (option) => {
    return (
      <div className="col-12 flex align-items-center auto-complete-items">
        <div className="col-3">{option?.StoneCode}</div>
        <div className="col-2">{option?.StoneTypeId?.StoneTypeName}</div>
        <div className="col-1">{option?.StoneQualityId?.StoneQualityName}</div>
        <div className="col-2">{option?.StoneCutTypeId?.StoneCutTypeName}</div>
        <div className="col-2">{option?.StoneShapeId?.StoneShapeName}</div>
        <div className="col-2">{option?.StoneSizeId?.StoneSizeName}</div>
      </div>
    );
  };
  ...
  const stoneFooterTemplate = () => {
    return (
      <div className="col-12 flex align-items-center auto-complete-items-footer">
        ...
        <div className="col-1">Quality</div>
        <div className="col-2">Cut</div>
        <div className="col-2">Shape</div>
  ...
  const searchStoneData = (event) => {
    ...
    setTimeout(() => {
      ...
      if (!event.query.trim().length) {
        ...
        inputItems.forEach((element) => {
          let allFilteredData =
            _filteredStoneData && _filteredStoneData.length > 0
              ? _filteredStoneData
              : allStoneData;
          ...
          _filteredStoneData = allFilteredData?.filter((ele) => {
            return (
              (ele?.StoneCode &&
                ele?.StoneCode?.toLowerCase().includes(
                  element?.toLowerCase()
                )) ||
              (ele?.StoneCutTypeId?.StoneCutTypeName &&
                ele?.StoneCutTypeId?.StoneCutTypeName?.toLowerCase().includes(
                  element?.toLowerCase()
                )) ||
              (ele?.StoneQualityId?.StoneQualityName &&
                ele?.StoneQualityId?.StoneQualityName?.toLowerCase().includes(
                  element?.toLowerCase()
                )) ||
              (ele?.StoneShapeId?.StoneShapeName &&
                ele?.StoneShapeId?.StoneShapeName?.toLowerCase().includes(
                  element?.toLowerCase()
                )) ||
              (ele?.StoneSizeId?.StoneSizeName &&
                ele?.StoneSizeId?.StoneSizeName?.toLowerCase().includes(
                  element?.toLowerCase()
                )) ||
              (ele?.StoneTypeId?.StoneTypeName &&
                ele?.StoneTypeId?.StoneTypeName?.toLowerCase().includes(
                  element?.toLowerCase()
                ))
            );
          });
  ...
  const addCopyItem = async () => {
    ...
    let isSKUExist = allInventoryData.find(
      (ele) =>
        ele?.Id !== state?.Id &&
        ele?.Title?.trim().toLowerCase() === newSKU.NewSku?.trim().toLowerCase()
    );
    if (
      isSKUExist ||
      !newSKU.NewSku?.trim().length ||
      !newSKU.NewMFNSKU.trim().length
    ) {
      setSubmitted(true);
      setBlocked(false);
      setCopyIsDialog(true);
      ...
      await addItemToSPList(LISTS.INVENTORY.NAME, currentState)
        .then(async (res) => {
          ...
          allInventoryData.push({
            ...state,
            Title: newSKU?.NewSku,
            MFNSKU: newSKU?.NewSku,
          });
          setProducts(allInventoryData);
          setAllInventoryData(allInventoryData);
          setSelectedProduct([
            { ...state, Title: newSKU?.NewSku, MFNSKU: newSKU?.NewSku },
          ]);
  ...
  const getTotalCostForImportExportProducts = (stoneData, productItem) => {
    ...
    const totalCost =
      ...
      (productItem.LDMountCost
        ? parseFloat(parseFloat(productItem.LDMountCost).toFixed(2))
        : 0) +
      (productItem.MiscCost1
        ? parseFloat(parseFloat(productItem.MiscCost1).toFixed(2))
        : 0) +
      (productItem.MiscCost2
        ? parseFloat(parseFloat(productItem.MiscCost2).toFixed(2))
        : 0);
    ...
    for (const productItem of products) {
      ...
      for (const productField of selectedProductFieldsForImportExport) {
        if (productField === "SKU") {
          ...
          productDataObject[productField] =
            productItem[productField]?.MetalTypeName;
          ...
          productDataObject[productField] =
            productItem[productField]?.SubCatName;
        } else if (productField === "Vendor2") {
          productDataObject[productField] =
            productItem[productField]?.VendorName;
        } else if (productField === "Parentage") {
          productDataObject[productField] = productItem[productField]?.Title;
        } else if (productField === "RecoverableCost") {
          productDataObject[productField] = productItem[productField]?.Cost
            ? parseInt(productItem[productField]?.Cost)
            : null;
        } else if (
          productField === "AmzLaunch" ||
          productField === "OfficeQtyZeroDate" ||
          productField === "FBAQtyZeroDate" ||
          productField === "RetiredDate" ||
          productField === "WatchListDate"
        ) {
          productDataObject[productField] = productItem[productField]
            ? format(new Date(productItem[productField]), "MM/dd/yyyy")
            : null;
        } else if (productField === "ModifiedDate") {
          productDataObject[productField] = productItem?.Modified
            ? format(new Date(productItem?.Modified), "MM/dd/yyyy")
            : null;
        } else if (productField === "StoneDetails") {
          const stoneDetails = productItem[productField]
            ? JSON.parse(productItem[productField])
            : [];
          if (stoneDetails.length > 0) {
            for (let i = 0; i < 7; i++) {
              if (stoneDetails[i]?.stoneCode && stoneDetails[i]?.StQuantity) {
                productDataObject["StoneCode" + (i + 1)] =
                  stoneDetails[i]?.stoneCode;
                productDataObject["StoneQty" + (i + 1)] = parseInt(
                  stoneDetails[i]?.StQuantity
                );
                productDataObject["StoneCost" + (i + 1)] =
                  stoneDetails[i]?.StoneCost;
              } else {
          ...
          const totalCostValues = await getTotalCostForImportExportProducts(
            stoneDetails,
            productItem
          );
          ...
          productDataObject["AmazonSuggested"] = sellingPriceDetails?.amazon
            ?.Suggested
            ? sellingPriceDetails?.amazon?.Suggested
            : null;
          productDataObject["AmazonActual"] = sellingPriceDetails?.amazon
            ?.Actual
            ? sellingPriceDetails?.amazon?.Actual
            : null;
          productDataObject["PeoraSuggested"] = sellingPriceDetails?.peora
            ?.Suggested
            ? sellingPriceDetails?.peora?.Suggested
            : null;
          productDataObject["PeoraActual"] = sellingPriceDetails?.peora?.Actual
            ? sellingPriceDetails?.peora?.Actual
            : null;
          productDataObject["OravoSuggested"] = sellingPriceDetails?.oravo
            ?.Suggested
            ? sellingPriceDetails?.oravo?.Suggested
            : null;
          productDataObject["OravoActual"] = sellingPriceDetails?.oravo?.Actual
            ? sellingPriceDetails?.oravo?.Actual
            : null;
          productDataObject["SMSuggested"] = sellingPriceDetails?.sm?.Suggested
            ? sellingPriceDetails?.sm?.Suggested
            : null;
          productDataObject["SMActual"] = sellingPriceDetails?.sm?.Actual
            ? sellingPriceDetails?.sm?.Actual
            : null;
          productDataObject["ROSuggested"] = sellingPriceDetails?.ro?.Suggested
            ? sellingPriceDetails?.ro?.Suggested
            : null;
          productDataObject["ROActual"] = sellingPriceDetails?.ro?.Actual
            ? sellingPriceDetails?.ro?.Actual
            : null;
        } else {
          if (
            productField !== "StoneCode1" &&
  ...
  const downloadCSVWithExportedProductsData = async () => {
    ...
    await exportProductsData(
      csvData,
      "ExportedProductsData_" + Date.now() + ".csv",
  ...
  const onAllProductFieldChange = (e, isExport = false) => {
    if (e.checked) {
      ...
      if (isExport) {
        ...
        for (let sellingPriceField of PRODUCT_SELLING_PRICE_FIELDS_DATA) {
          if (
            sellingPriceField !== "AmazonSuggested" &&
      ...
      if (isExport) {
        removeStoneDetailsFields = removeStoneDetailsFields.filter(
          (item) => item !== "StoneDetails" && item !== "SellingPrice"
        );
      } else {
        removeStoneDetailsFields = removeStoneDetailsFields.filter(
          (item) =>
      ...
      setSelectedProductFieldsForImportExport(
        _selectedProductFieldsForImportExport
      );
  ...
  const onSelectProductFieldChange = (e, isExport = false) => {
    let _selectedProductFieldsForImportExport = [
      ...selectedProductFieldsForImportExport,
    ];
    ...
    if (e.checked) {
      ...
      if (e.value === "StoneDetails") {
        for (let stoneField of PRODUCT_STONE_DETAILS_FIELDS_DATA) {
          _selectedProductFieldsForImportExport =
            _selectedProductFieldsForImportExport.filter(
              (item) => item !== stoneField
            );
        }
      } else if (e.value === "SellingPrice") {
        for (let sellingPriceField of PRODUCT_SELLING_PRICE_FIELDS_DATA) {
  ...
  const _addBulkExcelDatas = async (
    arrayItem: ISKUData[],
    emptyItem: ISKUData[]
  ) => {
    const sp: SPFI = getSP();
    const list: IList = await sp.web.lists.getByTitle(LISTS.INVENTORY.NAME);

    const [batchedListBehavior, execute] = createBatch(list);
    list.using(batchedListBehavior);

    if (arrayItem.length) {
      for (let i: number = 0; arrayItem.length > i; i++) {
        await sp.web.lists
          .getByTitle(LISTS.INVENTORY.NAME)
          .items.getById(arrayItem[i].ID)
          .update({
            Photos: arrayItem[i].Photos,
          })
          .then(async (res: any) => {
            if (arrayItem.length === i + 1) {
              await execute();
              await getAllInventory();
              setArrSyncItems([...arrayItem]);
              setArrNotSyncItems([...emptyItem]);
              setIsLoader(false);
              setIsVisible(true);
            }
          })
  ...
  const _prepareArray = async () => {
    let _availableImagesArray: ISKUData[] = [];
    let _notAvailableImagesArray: ISKUData[] = [];
    let _curSKUArray: ISKUData[] = [];

    await _arrSku.forEach(async (data: ISKUData) => {
      let _curDataArray: any[] = [];

      _curDataArray = await refactorAllInventoryData.filter((val: any) => {
        return (
          val.Title.split("-")[0].toLowerCase() ===
          data.Title.toString().toLowerCase()
        );
      });

      if (_curDataArray.length) {
        _curDataArray.forEach((val: any) => {
          _curSKUArray.push({
            ID: val.Id,
            Photos: data.Photos,
            Title: val.Title,
    ...
    for (let i: number = 0; _curSKUArray.length > i; i++) {
      if (_curSKUArray[i].Photos) {
        _availableImagesArray.push({ ..._curSKUArray[i] });
      } else {
        _notAvailableImagesArray.push({ ..._curSKUArray[i] });
      }

      if (_curSKUArray.length === i + 1) {
        await _addBulkExcelDatas(
          [..._availableImagesArray],
          [..._notAvailableImagesArray]
        );
      }
    }
  };

  const onProductDataXLSXFileUpload = async (xlsxFile: any) => {
    _arrSku = [];

    let _curDomain: string = absoluteUrl.split("/sites")[0];
    let file: any = xlsxFile[0];
    let fileType: string = file.name.split(".")[1];
    let _uniqueArrayOfData: any[] = [];
    ...
    if (fileType.toLowerCase() === "xlsx") {
      ...
      const filteredData = _removeEmptyDatas.filter((row: any) => {
        return row.some((cell: any) => cell !== null && cell !== "");
      });

      if (filteredData.length) {
        if (filteredData[0][1].toLowerCase() === "sku") {
          ...
          for (let i: number = 0; _uniqueArrayOfData.length > i; i++) {
            let objSku: ISKUData = { ..._objData };

            if (i >= 1) {
              let curTitle: string = "";
              let curPhotosJSON: string = "";

              curTitle = _uniqueArrayOfData[i][1].toString().split("-")[0];

              if (masterPhotos.length) {
                for (let j: number = 0; masterPhotos.length > j; j++) {
                  if (
                    _uniqueArrayOfData[i][1]
                      .toString()
                      .split("-")[0]
                      .toLowerCase() ===
                    masterPhotos[j].FileLeafRef.split(".")[0].toLowerCase()
                  ) {
                    curPhotosJSON = JSON.stringify([
                      {
                        fileName: masterPhotos[j].FileLeafRef,
                        objectURL: _curDomain + masterPhotos[j].FileRef,
                      },
                    ]);
                  }

                  if (masterPhotos.length === j + 1) {
                    _arrSku.push({
                      ...objSku,
                      Title: curTitle,
                      Photos: curPhotosJSON,
                    });
                  }

                  if (
                    _uniqueArrayOfData.length === i + 1 &&
                    masterPhotos.length === j + 1
                  ) {
                    setIsUploadXLSXVisible(false);
                    setIsLoader(true);
                    await _prepareArray();
                  }
                }
              } else {
                _arrSku.push({ ...objSku, Title: curTitle });

                if (_uniqueArrayOfData.length === i + 1) {
                  setIsUploadXLSXVisible(false);
                  setIsLoader(true);
                  await _prepareArray();
                }
            ...
            if (_uniqueArrayOfData.length === 1) {
              setIsUploadXLSXVisible(false);
              showErrorToast("Please add the datas.");
            }
          }
        } else {
          setIsUploadXLSXVisible(false);
          showErrorToast("Please import the proper sku content file.");
        }
      } else {
        setIsUploadXLSXVisible(false);
        showErrorToast("Please import the proper xlsx file.");
      }
    } else {
      setIsUploadXLSXVisible(false);
      showErrorToast("Please import the xlsx file.");
    }
  };

  const _generateExcel = async () => {
    let _arrExport: IMasterExportDatas[] = [..._exportMasterArray];
    let _isExport: boolean = false;

    const workbook: any = await new Excel.Workbook();
    const worksheet: any = await workbook.addWorksheet("sheet");
    ...
    worksheet.columns = [
      ...
      { header: "Ring Size", key: "RingSize", width: 15 },
      { header: "Metal Type", key: "MetalType", width: 15 },
      { header: "Jewelry Qty", key: "JewelryQty", width: 15 },
      { header: "Stone No", key: "StoneNo", width: 15 },
      { header: "Type", key: "Type", width: 15 },
      { header: "Shape", key: "Shape", width: 15 },
      { header: "Size", key: "Size", width: 15 },
      { header: "Cutting", key: "Cutting", width: 15 },
      { header: "Pcs per jewelry", key: "PcsPerJewelry", width: 15 },
      { header: "Quality", key: "Quality", width: 15 },
      { header: "Total Stone Qty", key: "TotalStoneQty", width: 15 },
      { header: "Vendor Code 1", key: "VendorCode1", width: 15 },
    ];
    ...
    for (let i: number = 0; _arrExport.length > i; i++) {
      let _excelRow: any = worksheet.addRow({
        No: _arrExport[i].No,
        Sku: _arrExport[i].Sku,
        RefNo: _arrExport[i].RefNo,
        RingSize: _arrExport[i].RingSize,
        MetalType: _arrExport[i].MetalType,
        JewelryQty: _arrExport[i].JewelryQty,
        StoneNo: _arrExport[i].StoneNo,
        Type: _arrExport[i].Type,
        Shape: _arrExport[i].Shape,
        Size: _arrExport[i].Size,
        Cutting: _arrExport[i].Cutting,
        PcsPerJewelry: _arrExport[i].PcsPerJewelry,
        Quality: _arrExport[i].Quality,
        TotalStoneQty: _arrExport[i].TotalStoneQty,
        VendorCode1: _arrExport[i].VendorCode1,
      });

      _excelRow.eachCell({ includeEmpty: true }, (cell: any) => {
      ...
      if (_arrExport[i].Picture) {
        const response = await fetch(_arrExport[i].Picture);
        const buffer = await response.arrayBuffer();

        const imageId2 = workbook.addImage({
          base64: `data:image/jpg;base64,${Buffer.from(buffer).toString(
            "base64"
          )}`,
          extension: "jpg",
        });

        const cellReference = { col: 0, row: i + 1 };

        worksheet.addImage(imageId2, {
          tl: { col: cellReference.col + 0.05, row: cellReference.row + 0.05 },
          ext: { width: 100, height: 100 },
        });
      }

      if (_arrExport.length === i + 1) {
        _isExport = true;
      }
    }

    if (_isExport || _arrExport.length === 0) {
      ...
      workbook.xlsx
        ...
        .then((buffer: any) =>
          FileSaver.saveAs(
            new Blob([buffer]),
            `Product ${moment().format("MMM DD YYYY")}.xlsx`
          )
        )
        .catch((err: any) => {
          showErrorToast("Error writing excel export");
        });
    }
  };

  const _getExportPrepareArray = async () => {
    _exportMasterArray = [];

    if (_arrExportProduct.length) {
      for (let i: number = 0; _arrExportProduct.length > i; i++) {
        if (_arrExportProduct[i].StoneCode.length) {
          for (
            let j: number = 0;
            _arrExportProduct[i].StoneCode.length > j;
            j++
          ) {
            for (let k: number = 0; allStoneData.length > k; k++) {
              let _stoneNo: number = j + 1;
              let _totStone: number =
                _arrExportProduct[i].JewelryQty *
                _arrExportProduct[i].StoneCode[j].StQuantity;

              if (
                _arrExportProduct[i].StoneCode[j].stoneCode.toLowerCase() ===
                allStoneData[k].StoneCode.toLowerCase()
              ) {
                _exportMasterArray.push({
                  Picture: _arrExportProduct[i].Photos.length
                    ? _arrExportProduct[i].Photos[0].objectURL
                    : "",
                  No: Number(_arrExportProduct[i].No),
                  Sku: _arrExportProduct[i].Sku,
                  RefNo: _arrExportProduct[i].RefNo,
                  RingSize: _arrExportProduct[i].RingSize
                    ? Number(_arrExportProduct[i].RingSize)
                    : null,
                  MetalType: _arrExportProduct[i].MetalType,
                  JewelryQty: _arrExportProduct[i].JewelryQty
                    ? Number(_arrExportProduct[i].JewelryQty)
                    : null,
                  StoneNo: Number(_stoneNo),
                  Type: allStoneData[k].StoneTypeIdId
                    ? allStoneData[k].StoneTypeId.StoneTypeName
                    : "",
                  Shape: allStoneData[k].StoneShapeIdId
                    ? allStoneData[k].StoneShapeId.StoneShapeName
                    : "",
                  Size: allStoneData[k].StoneSizeIdId
                    ? allStoneData[k].StoneSizeId.StoneSizeName
                    : "",
                  Cutting: allStoneData[k].StoneCutTypeIdId
                    ? allStoneData[k].StoneCutTypeId.StoneCutTypeName
                    : "",
                  PcsPerJewelry: Number(
                    _arrExportProduct[i].StoneCode[j].StQuantity
                  ),
                  Quality: allStoneData[k].StoneQualityIdId
                    ? allStoneData[k].StoneQualityId.StoneQualityName
                    : "",
                  TotalStoneQty: Number(_totStone),
                  VendorCode1: allStoneData[k].VendorId1Id
                    ? allStoneData[k].VendorId1.VendorName
                    : "",
                });
              }

              if (
                _arrExportProduct.length === i + 1 &&
                _arrExportProduct[i].StoneCode.length === j + 1 &&
                allStoneData.length === k + 1
              ) {
                await _generateExcel();
              }
            }
          }
        } else if (_arrExportProduct.length === i + 1) {
          await _generateExcel();
        }
      }
    } else {
  ...
  const onProductDataXLSXFileExport = async (xlsxFile: any) => {
    _arrExportProduct = [];
    ...
    if (fileType.toLowerCase() === "xlsx") {
      ...
      if (filteredData.length) {
        if (
          filteredData[0][1]?.toLowerCase() === "sku" &&
          ...
          for (let i: number = 0; _uniqueArrayOfData.length > i; i++) {
            if (i >= 1) {
              for (
                let j: number = 0;
                refactorAllInventoryData.length > j;
                j++
              ) {
                if (_uniqueArrayOfData[i][1]) {
                  if (
                    _uniqueArrayOfData[i][1].toString().toLowerCase() ===
                    refactorAllInventoryData[j].Title.toLowerCase()
                  ) {
                    _arrExportProduct.push({
                      No: _count,
                      Sku: _uniqueArrayOfData[i][1]
                        ? _uniqueArrayOfData[i][1]
                        : "",
                      RefNo: refactorAllInventoryData[j].VendorRef1
                        ? refactorAllInventoryData[j].VendorRef1
                        : "",
                      RingSize: refactorAllInventoryData[j].RingSize
                        ? refactorAllInventoryData[j].RingSize
                        : null,
                      MetalType: refactorAllInventoryData[j].MetalType
                        ? refactorAllInventoryData[j].MetalType
                        : "",
                      JewelryQty: _uniqueArrayOfData[i][2]
                        ? _uniqueArrayOfData[i][2]
                        : 1,
                      StoneCode: refactorAllInventoryData[j].StoneDetails.length
                        ? refactorAllInventoryData[j].StoneDetails
                        : [],
                      Photos: refactorAllInventoryData[j].Photos
                        ? JSON.parse(refactorAllInventoryData[j].Photos)
                        : [],
                    });
                ...
                if (
                  _uniqueArrayOfData.length === i + 1 &&
                  refactorAllInventoryData.length === j + 1
                ) {
                  setIsExportXLSXVisible(false);
                  setIsLoader(true);
                  await _getExportPrepareArray();
                }
              }
            }
          ...
          setIsExportXLSXVisible(false);
          showErrorToast("Please import the proper sku and qty content file.");
        }
      } else {
        setIsExportXLSXVisible(false);
        showErrorToast("Please import the proper xlsx file.");
  ...
  const preparedStoneDetailsObject = (stoneCode, stoneQty, sequence) => {
    if (stoneCode && stoneQty) {
      const filteredStoneData = allStoneData.filter(
        (x) =>
          x?.StoneCode?.trim().toLowerCase() === stoneCode.trim().toLowerCase()
      );
      if (filteredStoneData && filteredStoneData.length > 0) {
        ...
        preparedStoneDataObject["StoneCost"] = parseFloat(
          (filteredStoneData[0].LanEa * parseInt(stoneQty)).toFixed(2)
        );
  ...
  const importProductsDataToList = async (resultData) => {
    ...
    for (const productItem of importProductsDataArray) {
      ...
      if (productItem.SKU) {
        ...
        if (productItem.Category) {
          const filteredCategory = refactorProductCategoryData.filter(
            (x) => x.CatName === productItem.Category
          );
        ...
        if (productItem.Vendor1) {
          const filteredVendor = refactorAllVendorData.filter(
            (x) => x.VendorName === productItem.Vendor1
          );
        ...
        if (productItem.MetalType) {
          const filteredMetalType = refactorMetalData.filter(
            (x) => x.MetalTypeName === productItem.MetalType
          );
        ...
        if (productItem.Length) {
          const filteredLength = refactorLengthData.filter(
            (x) => x.Title === productItem.Length
          );
        ...
        if (productItem.Width) {
          const filteredWidth = refactorWidthData.filter(
            (x) => x.Title === productItem.Width
          );
        ...
        if (productItem.BaseMetal) {
          const filteredBaseMetal = refactorAllBaseMetal.filter(
            (x) => x.Title === productItem.BaseMetal
          );
        ...
        if (productItem.BaseStone) {
          const filteredBaseStone = refactorAllBaseStone.filter(
            (x) => x.Title === productItem.BaseStone
          );
        ...
        if (productItem.BoxType) {
          const filteredBoxType = refactorAllBoxType.filter(
            (x) => x.Title === productItem.BoxType
          );
        ...
        if (productItem.InsertType) {
          const filteredInsertType = refactorAllInsertType.filter(
            (x) => x.Title === productItem.InsertType
          );
        ...
        if (productItem.ChainType) {
          const filteredChainType = refactorAllChainType.filter(
            (x) => x.Title === productItem.ChainType
          );
        ...
        if (productItem.BackingType) {
          const filteredBackingType = refactorAllBackingType.filter(
            (x) => x.Title === productItem.BackingType
          );
        ...
        if (productItem.Subcategory) {
          const filteredSubcategory = refactorProductSubCategoryData.filter(
            (x) => x.SubCatName === productItem.Subcategory
          );
        ...
        if (productItem.Vendor2) {
          const filteredVendor2 = refactorAllVendorData.filter(
            (x) => x.VendorName === productItem.Vendor2
          );
        ...
        if (productItem.Parentage) {
          const filteredParentage = refactorAllParentage.filter(
            (x) => x.Title === productItem.Parentage
          );
        ...
        if (productItem.RecoverableCost) {
          const filteredRecoverableCost = refactorAllRecoverableCost.filter(
            (x) => x.Cost === parseInt(productItem.RecoverableCost)
          );
          if (filteredRecoverableCost && filteredRecoverableCost.length > 0) {
            preparedProductItemData.RecoverableCostId =
              filteredRecoverableCost[0].Id;
          }
        }
        ...
        if (
          !productItem.Retired ||
          productItem.Retired == "" ||
          productItem.Retired == undefined
        ) {
          delete preparedProductItemData.Retired;
        }

        if (
          !productItem.WatchList ||
          productItem.WatchList == "" ||
          productItem.WatchList == undefined
        ) {
          delete preparedProductItemData.WatchList;
        }

        if (
          !productItem.Inactive ||
          productItem.Inactive == "" ||
          productItem.Inactive == undefined
        ) {
        ...
        if (
          productItem.RestockLevel == "" ||
          productItem.RestockLevel == undefined
        ) {
          delete preparedProductItemData.RestockLevel;
        }

        if (
          productItem.ReserveQty == "" ||
          productItem.ReserveQty == undefined
        ) {
        ...
        if (
          productItem.FOBMountingCost == "" ||
          productItem.FOBMountingCost == undefined
        ) {
        ...
        if (
          productItem.LDMountCost == "" ||
          productItem.LDMountCost == undefined
        ) {
        ...
        if (productItem.OfficeQtyZeroDate) {
          preparedProductItemData.OfficeQtyZeroDate = new Date(
            productItem.OfficeQtyZeroDate
          );
        } else {
        ...
        if (productItem.FBAQtyZeroDate) {
          preparedProductItemData.FBAQtyZeroDate = new Date(
            productItem.FBAQtyZeroDate
          );
        } else {
        ...
        if (productItem.RetiredDate) {
          preparedProductItemData.RetiredDate = new Date(
            productItem.RetiredDate
          );
        } else {
        ...
        if (productItem.WatchListDate) {
          preparedProductItemData.WatchListDate = new Date(
            productItem.WatchListDate
          );
        } else {
        ...
        if (productItem.StoneCode1 && productItem.StoneQty1) {
          const stoneObj = await preparedStoneDetailsObject(
            productItem.StoneCode1,
            productItem.StoneQty1,
            1
          );
          if (stoneObj) {
        ...
        if (productItem.StoneCode2 && productItem.StoneQty2) {
          const stoneObj = await preparedStoneDetailsObject(
            productItem.StoneCode2,
            productItem.StoneQty2,
            2
          );
          if (stoneObj) {
            preparedStoneDetailsData.push(stoneObj);
        ...
        if (productItem.StoneCode3 && productItem.StoneQty3) {
          const stoneObj = await preparedStoneDetailsObject(
            productItem.StoneCode3,
            productItem.StoneQty3,
            3
          );
          if (stoneObj) {
            preparedStoneDetailsData.push(stoneObj);
        ...
        if (productItem.StoneCode4 && productItem.StoneQty4) {
          const stoneObj = await preparedStoneDetailsObject(
            productItem.StoneCode4,
            productItem.StoneQty4,
            4
          );
          if (stoneObj) {
            preparedStoneDetailsData.push(stoneObj);
        ...
        if (productItem.StoneCode5 && productItem.StoneQty5) {
          const stoneObj = await preparedStoneDetailsObject(
            productItem.StoneCode5,
            productItem.StoneQty5,
            5
          );
          if (stoneObj) {
            preparedStoneDetailsData.push(stoneObj);
        ...
        if (productItem.StoneCode6 && productItem.StoneQty6) {
          const stoneObj = await preparedStoneDetailsObject(
            productItem.StoneCode6,
            productItem.StoneQty6,
            6
          );
          if (stoneObj) {
            preparedStoneDetailsData.push(stoneObj);
        ...
        if (productItem.StoneCode7 && productItem.StoneQty7) {
          const stoneObj = await preparedStoneDetailsObject(
            productItem.StoneCode7,
            productItem.StoneQty7,
            7
          );
          if (stoneObj) {
            preparedStoneDetailsData.push(stoneObj);
        ...
        if (
          ...
          const existingProductData = products.filter(
            (x) => x.Title === productItem.SKU
          )[0];
          ...
          if (productItem.RecoverableCost) {
            ...
            recoverableCost = existingProductData?.RecoverableCost?.Cost
              ? existingProductData?.RecoverableCost?.Cost
              : 0;
          }
          ...
          if (productItem.BaseMetal) {
            ...
            baseMetal = existingProductData?.BaseMetal?.Title
              ? existingProductData?.BaseMetal?.Title
              : "";
          }
          ...
          if (
            ...
            stoneDetailsData = existingProductData?.StoneDetails
              ? JSON.parse(existingProductData?.StoneDetails)
              : [];
          }

          if (productItem.LDMountCost) {
            dataObjectForCalculateTotalCost.LDMountCost =
              productItem.LDMountCost;
          } else {
            dataObjectForCalculateTotalCost.LDMountCost =
              existingProductData?.LDMountCost
                ? existingProductData?.LDMountCost
                : 0;
          }
          ...
          if (productItem.MiscCost1) {
            ...
            dataObjectForCalculateTotalCost.MiscCost1 =
              existingProductData?.MiscCost1
                ? existingProductData?.MiscCost1
                : 0;
          }
          ...
          if (productItem.MiscCost2) {
            ...
            dataObjectForCalculateTotalCost.MiscCost2 =
              existingProductData?.MiscCost2
                ? existingProductData?.MiscCost2
                : 0;
          }
          ...
          const totalCostValues = await getTotalCostForImportExportProducts(
            stoneDetailsData,
            dataObjectForCalculateTotalCost
          );
          preparedSellingPriceData.amazon.Suggested =
            await getChannelsSuggestedPrice(
              "Amazon",
              baseMetal,
              parseInt(recoverableCost ? recoverableCost : 0),
              totalCostValues?.TotalCost
            );
          preparedSellingPriceData.peora.Suggested =
            await getChannelsSuggestedPrice(
              "Peora",
              baseMetal,
              parseInt(recoverableCost ? recoverableCost : 0),
          ...
          preparedSellingPriceData.oravo.Suggested =
            await getChannelsSuggestedPrice(
              "Oravo",
              null,
              0,
          ...
          preparedSellingPriceData.sm.Suggested =
            await getChannelsSuggestedPrice(
              "SM",
              null,
              0,
          ...
          preparedSellingPriceData.ro.Suggested =
            await getChannelsSuggestedPrice(
              "RO",
              baseMetal,
              parseInt(recoverableCost ? recoverableCost : 0),
          ...
          preparedProductItemData.SellingPrice = JSON.stringify(
            preparedSellingPriceData
          );
        }

        if (preparedStoneDetailsData.length > 0) {
          preparedStoneDetailsData.map(
            (stoneElement, index) => (stoneElement.Sequence = index + 1)
          );
          preparedProductItemData.StoneDetails = JSON.stringify(
            preparedStoneDetailsData
          );
        }

        if (
          productItem.AmazonActual ||
        ...
        if (
          ...
          const existingProductDataForSellingPrice = products.filter(
            (x) => x.Title === productItem.SKU
          )[0];
          const parseExistingSellingPrice = existingProductDataForSellingPrice
            ? JSON.parse(existingProductDataForSellingPrice?.SellingPrice)
            : preparedSellingPriceData;
          preparedSellingPriceData.amazon.Actual =
            parseExistingSellingPrice?.amazon?.Actual;
          preparedSellingPriceData.peora.Actual =
            parseExistingSellingPrice?.peora?.Actual;
          preparedSellingPriceData.oravo.Actual =
            parseExistingSellingPrice?.oravo?.Actual;
          preparedSellingPriceData.sm.Actual =
            parseExistingSellingPrice?.sm?.Actual;
          preparedSellingPriceData.ro.Actual =
            parseExistingSellingPrice?.ro?.Actual;
          preparedProductItemData.SellingPrice = JSON.stringify(
            preparedSellingPriceData
          );
        }
        ...
        const ifProductDataExist = products.filter(
          (x) => x.Title === productItem.SKU
        );
    ...
    if (preparedProductsDataToUpdate.length > 0) {
      await updateBulkProductItems(
        preparedProductsDataToUpdate,
        preparedProductsDataToUpdate.length >= preparedProductsDataToAdd.length
          ? true
          : false
      );
    }
    ...
    if (preparedProductsDataToAdd.length > 0) {
      await addBulkProductItems(
        preparedProductsDataToAdd,
        preparedProductsDataToAdd.length >= preparedProductsDataToUpdate.length
          ? true
          : false
      );
    }
    ...
    if (
      preparedProductsDataToUpdate.length == 0 &&
      preparedProductsDataToAdd.length == 0
    ) {
      setBlocked(false);
      setIsUploadCSVisible(false);
    }
    ...
    Papa.parse(file, {
      ...
      complete: async function (results) {
        if (results?.data?.length <= 500) {
          ...
          await importProductsDataToList(results?.data);
        } else {
    ...
    if (selectedProduct && selectedProduct.length > 0) {
      if (selectedProduct[0].StoneDetails) {
        ...
        for (const stone of stoneData) {
          const findStoneData = allStoneData.filter(
            (x) => x.Id === stone.stoneId
          )[0];

          stone["WrEaRdTotal"] = parseFloat(
            (findStoneData.WtEaRd * parseInt(stone.StQuantity)).toFixed(2)
          );
          stone["StoneCost"] = parseFloat(
            (findStoneData.LanEa * parseInt(stone.StQuantity)).toFixed(2)
          );
      ...
      if (selectedProduct[0].SellingPrice) {
        ...
        if (sellingData) {
          setAmazonPrice(
            sellingData.amazon ? sellingData.amazon : sellingInitialState
          );
          setPeoraPrice(
            sellingData.peora ? sellingData.peora : sellingInitialState
          );
          setOravoPrice(
            sellingData.oravo ? sellingData.oravo : sellingInitialState
          );
      ...
      if (selectedProduct[0].Photos) {
        ...
        setUploadedImages(
          uploadedImages && uploadedImages.length > 0 ? uploadedImages : null
        );
        setSelectedImage(
          uploadedImages && uploadedImages.length > 0
            ? uploadedImages[0]?.objectURL
      ...
      selectedProduct[0].Created = selectedProduct[0].Created
        ? new Date(selectedProduct[0].Created)
        : null;
      selectedProduct[0].AmzLaunch = selectedProduct[0].AmzLaunch
        ? new Date(selectedProduct[0].AmzLaunch)
        : null;
      selectedProduct[0].Modified = selectedProduct[0].Modified
        ? new Date(selectedProduct[0].Modified)
        : null;
      selectedProduct[0].OfficeQtyZeroDate = selectedProduct[0]
        .OfficeQtyZeroDate
        ? new Date(selectedProduct[0].OfficeQtyZeroDate)
        : null;
      selectedProduct[0].FBAQtyZeroDate = selectedProduct[0].FBAQtyZeroDate
        ? new Date(selectedProduct[0].FBAQtyZeroDate)
        : null;
      selectedProduct[0].RetiredDate = selectedProduct[0].RetiredDate
        ? new Date(selectedProduct[0].RetiredDate)
        : null;
      selectedProduct[0].WatchListDate = selectedProduct[0].WatchListDate
        ? new Date(selectedProduct[0].WatchListDate)
        : null;
      setIsStatusActive(selectedProduct[0]?.Inactive);
      setState({
        ...selectedProduct[0],
        Gender: genders.filter((x) => x?.code == selectedProduct[0].Gender)[0],
      });
    } else {
      handleNew();
    }
  ...
  const productHeader = (
    <div className="flex justify-content-between align-items-center">
      <h2 className="mx-2">
        SKU: {selectedProduct && selectedProduct[0]?.Title}
      </h2>
  ...
  const importFooterContent = (
    <div>
      ...
      />
      <CSVLink
        data={[finalFieldsForImportExport]}
        target="_blank"
        className="hidden"
  ...
  const exportFooterContent = (
    <div>
      ...
      <Button
        label="Export Data"
        icon="pi pi-file-excel"
        onClick={() => downloadCSVWithExportedProductsData()}
        autoFocus
      />
    </div>
  );

  const _closeSkuContent = (
    <div>
      <Button
        label="Close"
        icon="pi pi-times-circle"
        onClick={() => {
          setIsOpenSkuNo(false);
        }}
        autoFocus
      />
    </div>
    ...
    <div className="p-fluid grid">
      <div className="col-2 md:col-2 lg:col-2 m-0">
        <div
          ...
          style={{ minHeight: gridViewHeight }}
        >
          <DataTable
            className="sku-data-table"
            value={products}
            scrollable
            scrollHeight={gridViewHeight - 10 + "px"}
            selectionMode="single"
            filters={globalFilter}
            showGridlines
            header={header}
            selection={selectedProduct}
            onSelectionChange={(e) => selectedProductData(e.value)}
      ...
      <div ref={contentDivRef} className="col-10 md:col-10 lg:col-10">
                  ...
                  <Fieldset className="p-0 m-0">
                      ...
                      <div className="flex">
                        <div className="col-12 md:col-4 lg:col-4">
                          <label htmlFor="SKU" className="font-bold block my-2">
                            SKU
                            <span className="required" hidden={isStatusActive}>
                              {" "}
                              *
                            </span>
                          </label>
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <InputText
                            ...
                            className={classNames(
                              {
                                "p-invalid":
                                  submitted && !state?.Title?.trim().length,
                              },
                              "w-full"
                            )}
                          />
                          {submitted && !state?.Title?.trim().length && (
                            <small className="p-error">SKU is required.</small>
                          )}
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <Dropdown
                            ...
                            options={refactorProductCategoryData}
                            ...
                            disabled={isStatusActive}
                            className={classNames(
                              { "p-invalid": submitted && !state?.Category },
                              "w-full"
                            )}
                            showClear
                            filter
                          />
                          {submitted && !state?.Category && (
                      ...
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <InputNumber
                            value={state?.MtWtFOB}
                            min={0}
                            onChange={(e) => handleChangeNumber(e, "MtWtFOB")}
                            id="MtWtFOB"
                            name="MtWtFOB"
                            minFractionDigits={2}
                            maxFractionDigits={2}
                    ...
                    <div className="col-12 md:col-12 lg:col-12 p-0">
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <InputNumber
                            value={state?.MtWtFeed}
                            min={0}
                            onChange={(e) => handleChangeNumber(e, "MtWtFeed")}
                            id="MtWtFeed"
                            name="MtWtFeed"
                            minFractionDigits={2}
                            maxFractionDigits={2}
                      ...
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <Dropdown
                            ...
                            options={refactorMetalData}
                            onChange={handleDropdownChange}
                            id="MetalType"
                            name="MetalType"
                            optionLabel="MetalTypeName"
                            disabled={isStatusActive}
                            className={classNames(
                              { "p-invalid": submitted && !state?.MetalType },
                              "w-full"
                            )}
                            showClear
                            filter
                          />
                          {submitted && !state?.MetalType && (
                            <small className="p-error">
                          ...
                          <div className="flex flex-wrap gap-3 mt-1">
                            <div className="flex align-items-center">
                              <RadioButton
                                ...
                                value={true}
                                onChange={(e) => handleChange(e)}
                                checked={state?.OnFBA === true}
                              />
                              <label htmlFor="OnFBA1" className="ml-2">
                                Yes
                              </label>
                            </div>
                            <div className="flex align-items-center">
                              <RadioButton
                                ...
                                value={false}
                                onChange={(e) => handleChange(e)}
                                checked={state?.OnFBA === false}
                              />
                              <label htmlFor="OnFBA2" className="ml-2">
                                No
                              </label>
                            </div>
                          </div>
                      ...
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <InputNumber
                            ...
                            min={0}
                            onChange={(e) => handleChangeNumber(e, "TierLevel")}
                            id="TierLevel"
                            name="TierLevel"
                            disabled={isStatusActive}
                          />
                    ...
                    <div className="col-12 md:col-12 lg:col-12 p-0">
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <Dropdown
                            resetFilterOnHide={true}
                            value={state?.RecoverableCost}
                            options={refactorAllRecoverableCost}
                            onChange={handleDropdownChange}
                            id="RecoverableCost"
                  ...
                  <Fieldset className="p-0 m-0">
                    <div className="col-12 md:col-12 lg:col-12 p-0">
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <InputNumber
                            value={state?.OfficeQty}
                            onChange={(e) => {
                              setState({ ...state, OfficeQty: e.value });
                            }}
                            id="OfficeQty"
                            name="OfficeQty"
                            disabled={isStatusActive}
                      ...
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <InputNumber
                            value={state?.OnOrder}
                            onChange={(e) => {
                              setState({ ...state, OnOrder: e.value });
                            }}
                            id="OnOrder"
                            name="OnOrder"
                            disabled={isStatusActive}
                    ...
                    <div className="col-12 md:col-12 lg:col-12 p-0">
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <InputNumber
                            value={state?.RestockLevel}
                            min={0}
                            onChange={(e) => {
                              setState({ ...state, RestockLevel: e.value });
                            }}
                            id="RestockLevel"
                            name="RestockLevel"
                            disabled={isStatusActive}
                    ...
                    <div className="col-12 md:col-12 lg:col-12 p-0">
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <InputNumber
                            value={state?.ReserveQty}
                            min={0}
                            onChange={(e) => {
                              setState({ ...state, ReserveQty: e.value });
                            }}
                            id="ReserveQty"
                            name="ReserveQty"
                            disabled={isStatusActive}
                  ...
                  <Fieldset className="p-0 m-0 mt-2">
                    ...
                    <div className="col-12 md:col-12 lg:col-12 p-0">
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <Calendar
                            value={state?.OfficeQtyZeroDate}
                            onChange={(e) => handleChange(e)}
                            id="OfficeQtyZeroDate"
                            name="OfficeQtyZeroDate"
                            showIcon
                            disabled={isStatusActive}
                          />
                          {/* {submitted && !state?.VendorRef1?.trim().length && <small className="p-error">Vendor Ref is required.</small> ----  className={classNames({ 'p-invalid': submitted && !state?.VendorRef1?.trim().length }, "w-full")} } */}
                      ...
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <InputNumber
                            value={state?.Return}
                            min={0}
                            onChange={(e) => handleChangeNumber(e, "Return")}
                            id="Return"
                            name="Return"
                            minFractionDigits={2}
                            maxFractionDigits={2}
                      ...
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <InputNumber
                            ...
                            min={0}
                            onChange={(e) => handleChangeNumber(e, "RingSize")}
                            id="RingSize"
                            name="RingSize"
                            minFractionDigits={1}
                            maxFractionDigits={1}
                ...
                <div className="col-12 md:col-6 lg:col-6 p-0">
                  <div className="col-12 md:col-12 lg:col-12 p-0 flex">
                    <div className="col-12 md:col-6 lg:col-6">
                      <Fieldset className="p-0 m-0">
                        <div
                          className="col-12 md:col-12 lg:col-12 p-0"
                          ref={productPriceRef}
                        >
                          <div className="col-12 md:col-12 lg:col-12 p-0">
                            <div className="flex">
                          ...
                          <div className="col-12 md:col-12 lg:col-12 p-0">
                            <div className="flex">
                              ...
                              <div className="col-12 md:col-4 lg:col-4 pr-1">
                                <InputNumber
                                  min={0}
                                  value={amazonPrice.Suggested}
                                  onValueChange={(e) => handleChangePrice(e)}
                                  id="amazonSuggested"
                                  name="Suggested"
                                  minFractionDigits={2}
                                  maxFractionDigits={2}
                    ...
                    <div className="col-12 md:col-6 lg:col-6">
                      <Fieldset
                        className="p-0 m-0 flex align-items-center justify-content-center"
                            ...
                            <div className="col-12 md:col-12 lg:col-12 p-0 text-center image-data">
                              ...
                              <div className="flex col-12 md:col-12 lg:col-12 text-left p-0 pt-2">
                                {uploadedImages &&
                                  ...
                                  uploadedImages.map((x, index) => {
                                    if (index < 4) {
                                      return (
                                        <div
                                          className="text-center image-border uploaded-image"
                                          onClick={(e) =>
                                            setSelectedImage(x?.objectURL)
                                          }
                                        >
                                          <Image
                                            src={x?.objectURL}
                                ...
                                <FileUpload
                                  ...
                                  auto
                                  chooseLabel=""
                                  onUpload={(e) => onFileUpload(e.files)}
                                  onRemove={(e) => onFileRemove(e, "Logo")}
                      ...
                      <div className="col-12 md:col-12 lg:col-12 p-0 flex">
                        <div className="col-6 md:col-6 lg:col-6 p-0">
                          <div className="flex">
                            ...
                            <div className="col-12 md:col-8 lg:col-8">
                              <InputNumber
                                value={state?.FOBMountingCost}
                                min={0}
                                onChange={(e) =>
                                  handleChangeNumber(e, "FOBMountingCost")
                                }
                                id="FOBMountingCost"
                                name="FOBMountingCost"
                                minFractionDigits={2}
                                maxFractionDigits={2}
                                disabled={isStatusActive}
                              />
                        ...
                        <div className="col-6 md:col-6 lg:col-6 p-0">
                          <div className="flex">
                            ...
                            <div className="col-12 md:col-8 lg:col-8">
                              <InputNumber
                                value={state?.LDMountCost}
                                min={0}
                                onChange={(e) =>
                                  handleChangeNumber(e, "LDMountCost")
                                }
                                id="LDMountCost"
                                name="LDMountCost"
                                minFractionDigits={2}
                      ...
                      <div className="col-12 md:col-12 lg:col-12 p-0 flex">
                        ...
                        <div className="col-6 md:col-6 lg:col-6 p-0">
                          <div className="flex">
                            ...
                            <div className="col-12 md:col-8 lg:col-8">
                              <InputNumber
                                value={state?.MiscCost1}
                                min={0}
                                onChange={(e) =>
                                  handleChangeNumber(e, "MiscCost1")
                                }
                                id="MiscCost1"
                                name="MiscCost1"
                                minFractionDigits={2}
                      ...
                      <div className="col-12 md:col-12 lg:col-12 p-0 flex">
                        ...
                        <div className="col-6 md:col-6 lg:col-6 p-0">
                          <div className="flex">
                            ...
                            <div className="col-12 md:col-8 lg:col-8">
                              <InputNumber
                                value={state?.MiscCost2}
                                min={0}
                                onChange={(e) =>
                                  handleChangeNumber(e, "MiscCost2")
                                }
                                id="MiscCost2"
                                name="MiscCost2"
                                minFractionDigits={2}
                  ...
                  <Fieldset className="p-0 m-0">
                    <div className="col-12 md:col-12 lg:col-12 flex">
                      <AutoComplete
                        field="StoneCode"
                        value={selectedStone}
                        suggestions={filteredStoneData}
                        className="stone-auto-complete"
                        completeMethod={searchStoneData}
                        onChange={(e) => handleStoneChange(e)}
                        itemTemplate={stoneTemplate}
                        panelFooterTemplate={stoneFooterTemplate}
                      />
                      ...
                      <InputNumber
                        ...
                        onChange={(e) => setSelectedStoneQuantity(e.value)}
                        id="selectedStoneQuantity"
                        name="selectedStoneQuantity"
                        disabled={isStatusActive}
                      />
                      <Button
                        className="ml-2"
                    ...
                    <DataTable
                      value={selectedStonesData}
                      footerColumnGroup={footerGroup}
                      showGridlines
                      className="stone-table"
                    >
                      <Column
                        body={currentProductOptionTemplate}
                      ...
                      <Column
                        ...
                        field="StQuantity"
                        header="Pcs Per"
                        editor={(options) => inputNumber(options)}
                        onCellEditComplete={onRowEditComplete}
                        cellEditValidator={onCellEditValidator}
                      ></Column>
                      <Column
                        className="text-right"
                        field="WtEa"
                      ...
                      <Column
                        ...
                        field="FOBEa"
                        header="$/FOB Ea"
                        body={fobBodyTemplate}
                      ></Column>
                      <Column
                        className="text-right"
                        field="LanEa"
                        header="$/Lan Ea"
                        body={lanBodyTemplate}
                      ></Column>
                      <Column
                        className="text-right"
                        field="StoneCost"
                        header="Stone Cost"
                        body={stoneCostBodyTemplate}
                      ></Column>
                    </DataTable>
                      ...
                      <div className="col-12 col-offset-8 md:col-4 lg:col-4 p-0">
                        <div className="flex">
                          <div className="col-12 md:col-4 lg:col-4">
                            <label
                              htmlFor="MiscCost2"
                              className="font-bold block my-2 total-label"
                            >
                              Total Unit Cost
                            </label>
                          </div>
                    ...
                    <div className="col-12 md:col-12 lg:col-12 p-0">
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <Dropdown
                            resetFilterOnHide={true}
                            value={state?.BaseStone}
                            options={refactorAllBaseStone}
                            onChange={handleDropdownChange}
                            id="BaseStone"
                            name="BaseStone"
                            optionLabel="Title"
                    ...
                    <div className="col-12 md:col-12 lg:col-12 p-0">
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <Dropdown
                            resetFilterOnHide={true}
                            value={state?.Parentage}
                            options={refactorAllParentage}
                            onChange={handleDropdownChange}
                            id="Parentage"
                            name="Parentage"
                            optionLabel="Title"
                          ...
                          <div className="flex flex-wrap gap-3 mt-1">
                            <div className="flex align-items-center">
                              <RadioButton
                                ...
                                value={true}
                                onChange={(e) => handleChange(e)}
                                checked={state?.VIPSKU === true}
                              />
                              <label htmlFor="VIPSKU1" className="ml-2">
                                Yes
                              </label>
                            </div>
                            <div className="flex align-items-center">
                              <RadioButton
                                ...
                                value={false}
                                onChange={(e) => handleChange(e)}
                                checked={state?.VIPSKU === false}
                              />
                              <label htmlFor="VIPSKU2" className="ml-2">
                                No
                              </label>
                            </div>
                          </div>
                    ...
                    <div className="col-12 md:col-12 lg:col-12 p-0">
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <Dropdown
                            resetFilterOnHide={true}
                            value={state?.InsertType}
                            options={refactorAllInsertType}
                            onChange={handleDropdownChange}
                            id="InsertType"
                            name="InsertType"
                            optionLabel="Title"
                    ...
                    <div className="col-12 md:col-12 lg:col-12 p-0">
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <Dropdown
                            resetFilterOnHide={true}
                            value={state?.BackingType}
                            options={refactorAllBackingType}
                            onChange={handleDropdownChange}
                            id="BackingType"
                            name="BackingType"
                            optionLabel="Title"
                      ...
                      <div className="flex">
                        ...
                        <div className="col-12 md:col-8 lg:col-8">
                          <Dropdown
                            ...
                            options={refactorProductSubCategoryData}
                            onChange={handleDropdownChange}
                            id="Subcategory"
                            name="Subcategory"
                            optionLabel="SubCatName"
                            showClear
                            filter
                            disabled={isStatusActive}
                          />
                            ...
                            <div className="flex flex-wrap gap-3 mt-1">
                              <div className="flex align-items-center">
                                <RadioButton
                                  ...
                                  value={true}
                                  onChange={(e) => handleChange(e)}
                                  checked={state?.Retired === true}
                                />
                                <label htmlFor="Retired1" className="ml-2">
                                  Yes
                                </label>
                              </div>
                              <div className="flex align-items-center">
                                <RadioButton
                                  ...
                                  value={false}
                                  onChange={(e) => handleChange(e)}
                                  checked={state?.Retired === false}
                                />
                                <label htmlFor="Retired2" className="ml-2">
                                  No
                                </label>
                              </div>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-3 mt-1">
                              <div className="flex align-items-center">
                                <RadioButton
                                  ...
                                  value={true}
                                  onChange={(e) => handleChange(e)}
                                  checked={state?.WatchList === true}
                                />
                                <label htmlFor="WatchList1" className="ml-2">
                                  Yes
                                </label>
                              </div>
                              <div className="flex align-items-center">
                                <RadioButton
                                  ...
                                  value={false}
                                  onChange={(e) => handleChange(e)}
                                  checked={state?.WatchList === false}
                                />
                                <label htmlFor="WatchList2" className="ml-2">
                                  No
                                </label>
                              </div>
                            </div>
        ...
        <Dialog
          visible={isCopyDialog}
          style={{ width: "450px" }}
          footer={copyFooterComponent}
          header="Confirm"
          modal
          onHide={onHideCopyDialog}
        >
          <div className="p-fluid grid">
            <div className="col-12 md:col-12 lg:col-12 p-0">
              <div className="flex">
        ...
        <Dialog
          visible={isPrintDialog}
          style={{ width: "45.5%" }}
          footer={printFooterComponent}
          header="Print SKUs"
          modal
          onHide={() => setIsPrintDialog(false)}
        >
          <div className="p-fluid grid">
            <div className="col-12 md:col-12 lg:col-12">
              ...
              <AutoComplete
                className={classNames({
                  "p-invalid": isPrintValidation && selectedSKUData.length <= 0,
                })}
                field="Title"
                multiple
              ...
              />
              {isPrintValidation && selectedSKUData.length <= 0 && (
                <small className="p-error">
        ...
        >
          ...
          <div className="col-12 md:col-12 lg:col-12">
            <Checkbox
              inputId="AllProductFields"
              name="AllProductFields"
              value="AllProductFields"
              onChange={(e) => onAllProductFieldChange(e)}
              checked={allProductFieldChecked}
            />
            <label htmlFor="AllProductFields" className="ml-2">
              All
            </label>
          </div>
          <div className="col-12 md:col-12 lg:col-12">
                ...
                return (
                  item !== "ModifiedDate" && (
                    <div key={item} className="flex align-items-center w-9rem">
                      <Checkbox
                        inputId={item}
                        name="product-field-list"
                        value={item}
                        onChange={(e) => onSelectProductFieldChange(e)}
                        checked={selectedProductFieldsForImportExport.some(
                          (x) => x === item
                        )}
                      />
        ...
        >
          ...
          <div className="col-12 md:col-12 lg:col-12">
            <Checkbox
              ...
              name="AllProductFields"
              value="AllProductFields"
              onChange={(e) => onAllProductFieldChange(e, true)}
              checked={allProductFieldChecked}
            />
            <label htmlFor="AllProductFields" className="ml-2">
              All
                ...
                return (
                  <div key={item} className="flex align-items-center w-9rem">
                    <Checkbox
                      ...
                      name="product-field-list"
                      value={item}
                      onChange={(e) => onSelectProductFieldChange(e, true)}
                      checked={selectedProductFieldsForImportExport.some(
                        (x) => x === item
                      )}
                    />
        ...
        <Dialog
          header="Upload Product CSV"
          visible={isUploadCSVisible}
          style={{ width: "54vw" }}
          onHide={() => setIsUploadCSVisible(false)}
        >
          <div className="col-12 text-right">
            <Button
              label={"Generate CSV File for upload data."}
              icon="pi pi-database"
              tooltip={"Import"}
              onClick={() => {
                onClickGenerateCSVFile();
              }}
              data-pr-position="bottom"
            />
          </div>
          <div className="col-12">
            <FileUpload
              name="UploadProductDataCSV"
              onUpload={(e) => onProductDataCSVFileUpload(e.files)}
              url="/"
            />
            <small className="p-error">
              Note: Please import maximum 500 products at a time.
            </small>
          </div>
        </Dialog>
        ...
        <Dialog
          ...
          visible={isUploadXLSXVisible}
          style={{ width: "54vw" }}
          onHide={() => setIsUploadXLSXVisible(false)}
        >
          <div className="col-12">
            <FileUpload
              name="Upload Product Data XLSX"
              onUpload={(e) => onProductDataXLSXFileUpload(e.files)}
              url="/"
            />
          </div>
        </Dialog>
        ...
        <Dialog
          header="Export Product"
          visible={isExportXLSXVisible}
          style={{ width: "54vw" }}
          onHide={() => setIsExportXLSXVisible(false)}
        >
          <div className="col-12">
            <FileUpload
              name="Export Product Data XLSX"
              onUpload={(e) => onProductDataXLSXFileExport(e.files)}
              url="/"
            />
          </div>
        </Dialog>
        ...
        <Dialog
          ...
          visible={isVisible}
          position="center"
          style={{ width: "25vw", minHeight: "112px" }}
          onHide={() => {
            setIsVisible(false);
          }}
          id="AlertDialog"
        ...
        >
          {arrSyncItems.length ? (
            ...
            >
              {arrSyncItems.length + " Product(s) updated successfully"}
            </div>
          ) : (
            ""
          )}

          {arrNotSyncItems.length ? (
            <div>
              <div
                style={{
                  fontSize: "16px",
              ...
              >
                {arrNotSyncItems.length + " Product(s) not updated"}
                <span
                  style={{
                    fontSize: "14px",
                    paddingLeft: "20px",
        ...
        <Dialog
          ...
          footer={_closeSkuContent}
          id="notUpdateSku"
          closable={false}
          draggable={false}
          resizable={false}
        >
          {arrNotSyncItems.map((val: ISKUData) => {
