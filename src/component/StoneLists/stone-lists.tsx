...
const StoneListsView = () => {
  ...
  const downloadCSVWithData = async () => {
    ...
    await exportData(
      csvData,
      "ExportedStoneData_" + Date.now() + ".csv",
  ...
  const uploadDataToList = async (resultData) => {
    setBlocked(true);
    ...
    for (const item of uploadData) {
      ...
      if (item.StoneCode) {
        if (item.StoneTypeId) {
          const filteredStoneType = refactorStoneTypeData.filter(
            (x) => x.StoneTypeName === item.StoneTypeId
          );
        ...
        if (item.StoneShapeId) {
          const filteredStoneShape = refactorStoneShapeData.filter(
            (x) => x.StoneShapeName === item.StoneShapeId
          );
        ...
        if (item.StoneSizeId) {
          const filteredStoneSize = refactorStoneSizeData.filter(
            (x) => x.StoneSizeName === item.StoneSizeId
          );
        ...
        if (item.StoneCutTypeId) {
          const filteredStoneCutType = refactorStoneCutTypeData.filter(
            (x) => x.StoneCutTypeName === item.StoneCutTypeId
          );
        ...
        if (item.StoneQualityId) {
          const filteredStoneQuality = refactorStoneQualityData.filter(
            (x) => x.StoneQualityName === item.StoneQualityId
          );
        ...
        //     ? true
        //     : false;
        // itemData.Pct = item.Pct ? Number(item.Pct) : null;
        // itemData.WtEa = item.WtEa ? Number(item.WtEa) : null;
        // itemData.WtEaRd = item.WtEaRd ? Number(item.WtEaRd) : null;
        // itemData.FOBEa = item.FOBEa ? Number(item.FOBEa) : null;
        // itemData.LanEa = item.LanEa ? Number(item.LanEa) : null;
        ...
        const ifStoneData = allStoneData.filter(
          (x) => x.StoneCode === item.StoneCode
        );
    ...
    if (preparedStoneDataToUpdate.length > 0) {
      await updateBulkStonesItem(
        preparedStoneDataToUpdate,
        preparedStoneDataToUpdate.length >= preparedStoneDataToAdd.length
          ? true
          : false
      );
    }
    ...
    if (preparedStoneDataToAdd.length > 0) {
      await addBulkStonesItem(
        preparedStoneDataToAdd,
        preparedStoneDataToAdd.length >= preparedStoneDataToUpdate.length
          ? true
          : false
      );
    }
    ...
    if (
      preparedStoneDataToUpdate.length == 0 &&
      preparedStoneDataToAdd.length == 0
    ) {
      setBlocked(false);
      setIsUploadCSVisible(false);
    }
  ...
  useEffect(() => {
    setBlocked(true);
    setStoneDetailsSectionHeight(
      stoneDetailsSectionHeightRef.current.clientHeight + 113
    );
  ...
  const handleWtEaChange = (e) => {
    ...
    state.FOBEa =
      value && state.Pct
        ? parseFloat((parseFloat(value.toFixed(3)) * state.Pct).toFixed(3))
        : null;
    setState({
      ...state,
      [name]: value,
  ...
  const handlePctChange = (e) => {
    ...
    state.FOBEa =
      value && state.WtEa
        ? parseFloat((parseFloat(state.WtEa.toFixed(3)) * value).toFixed(3))
        : null;
    setState({
      ...state,
      [name]: value,
  ...
  const handleStoneSubmit = async () => {
    setSubmitted(false);
    let isStoneExist = allStoneData.find(
      (ele) =>
        ele?.Id !== state?.Id &&
        ele?.StoneCode?.trim().toLowerCase() ===
          state.StoneCode?.trim().toLowerCase()
    );
    if (
      isStoneExist ||
      !state?.StoneCode?.trim().length ||
      !state?.StoneTypeIdId ||
      !state?.StoneShapeIdId ||
      !state?.StoneSizeIdId ||
      !state?.StoneCutTypeIdId ||
      !state?.StoneQualityIdId ||
      !state?.LanEa
    ) {
      setSubmitted(true);
    ...
    if (state?.Id) {
      updateSingleFieldValueToSPList(
        ...
        .then((res) => {
          ...
          const updateStoneData = allStoneData.map(
            ({ Inactive, ...stoneEle }) => ({
              ...stoneEle,
      ...
      <div className="col-12 md:col-12 lg:col-12 p-0">
        <span className="p-input-icon-left">
          ...
          <InputText
            ...
            className="w-full"
            type="search"
            value={filterStoneInputValue}
            onChange={(e) => searchStoneData(e)}
          />
        </span>
      </div>
    );
  ...
  const onCategoryChange = (e) => {
    ...
    if (e.checked) _selectedStones.push(e.value);
    else _selectedStones = _selectedStones.filter((item) => item !== e.value);
    ...
    Papa.parse(file, {
      ...
      complete: async function (results) {
        if (results?.data?.length <= 500) {
          ...
          await uploadDataToList(results?.data);
        } else {
  ...
  return (
    <div className="p-fluid grid">
      ...
      <div className="col-12 md:col-2 lg:col-2">
        <ListBox
          filterTemplate={allStoneFilterTemplate}
          className="w-full"
          filter
          value={stoneRowData}
          onChange={(e) => onStoneHandleChange(e)}
          options={filteredStoneData}
          optionLabel="StoneCode"
          filterPlaceholder=""
          emptyFilterMessage="Stone not available."
          listStyle={{
            height: `${stoneDetailsSectionHeight - 63}px`,
            overflow: "auto",
          }}
          style={{ height: `${stoneDetailsSectionHeight}px` }}
        />
      </div>
              ...
              <Fieldset className="p-0 m-0">
                    ...
                    <div className="col-12 md:col-8 lg:col-8">
                      <InputText
                        ...
                        className={classNames(
                          {
                            "p-invalid":
                              submitted && !state?.StoneCode?.trim().length,
                          },
                          "w-full"
                        )}
                        disabled={isReadOnly}
                      ...
                      {submitted && !state?.StoneCode?.trim().length && (
                        <small className="p-error">
                          Stone code is required.
                        </small>
                      )}
                      {stoneExist && state?.StoneCode?.trim().length && (
                        <small className="p-error">Stone already exists.</small>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="col-12 md:col-12 lg:col-12 p-0">
                  <div className="flex">
                    ...
                    <div className="col-12 md:col-8 lg:col-8">
                      <Dropdown
                        value={state?.StoneTypeId}
                        options={refactorStoneTypeData}
                        onChange={(e) => handleDropdownChange(e)}
                        name="StoneTypeId"
                        optionLabel="StoneTypeName"
                        className={classNames(
                          { "p-invalid": submitted && !state?.StoneTypeIdId },
                          "w-full"
                        )}
                ...
                <div className="col-12 md:col-12 lg:col-12 p-0">
                  <div className="flex">
                    ...
                    <div className="col-12 md:col-8 lg:col-8">
                      <Dropdown
                        value={state?.StoneShapeId}
                        options={refactorStoneShapeData}
                        onChange={(e) => handleDropdownChange(e)}
                        name="StoneShapeId"
                        optionLabel="StoneShapeName"
                        className={classNames(
                          { "p-invalid": submitted && !state?.StoneShapeIdId },
                          "w-full"
                        )}
                        resetFilterOnHide={true}
                        showClear
                        ...
                        disabled={isReadOnly}
                      />
                      {submitted && !state?.StoneShapeIdId && (
                        <small className="p-error">
                          Stone shape is required.
                        </small>
                      )}
                    </div>
                  </div>
                ...
                <div className="col-12 md:col-12 lg:col-12 p-0">
                  <div className="flex">
                    ...
                    <div className="col-12 md:col-8 lg:col-8">
                      <Dropdown
                        value={state?.StoneSizeId}
                        options={refactorStoneSizeData}
                        onChange={(e) => handleDropdownChange(e)}
                        name="StoneSizeId"
                        optionLabel="StoneSizeName"
                        className={classNames(
                          { "p-invalid": submitted && !state?.StoneSizeIdId },
                          "w-full"
                        )}
                        resetFilterOnHide={true}
                        showClear
                        ...
                        disabled={isReadOnly}
                      />
                      {submitted && !state?.StoneSizeIdId && (
                        <small className="p-error">
                          Stone size is required.
                        </small>
                      )}
                    </div>
                  </div>
                ...
                <div className="col-12 md:col-12 lg:col-12 p-0">
                  <div className="flex">
                    ...
                    <div className="col-12 md:col-8 lg:col-8">
                      <Dropdown
                        value={state?.StoneCutTypeId}
                        options={refactorStoneCutTypeData}
                        onChange={(e) => handleDropdownChange(e)}
                        name="StoneCutTypeId"
                        optionLabel="StoneCutTypeName"
                        className={classNames(
                          {
                            "p-invalid": submitted && !state?.StoneCutTypeIdId,
                          },
                          "w-full"
                        )}
                        resetFilterOnHide={true}
                        ...
                        disabled={isReadOnly}
                      />
                      {submitted && !state?.StoneCutTypeIdId && (
                        <small className="p-error">
                          Stone cut is required.
                        </small>
                      )}
                    </div>
                  </div>
                ...
                <div className="col-12 md:col-12 lg:col-12 p-0">
                  <div className="flex">
                    ...
                    <div className="col-12 md:col-8 lg:col-8">
                      <Dropdown
                        value={state?.StoneQualityId}
                        options={refactorStoneQualityData}
                        onChange={(e) => handleDropdownChange(e)}
                        name="StoneQualityId"
                        optionLabel="StoneQualityName"
                        className={classNames(
                          {
                            "p-invalid": submitted && !state?.StoneQualityIdId,
                          },
                          "w-full"
                        )}
                        resetFilterOnHide={true}
                        ...
                        disabled={isReadOnly}
                      />
                      {submitted && !state?.StoneQualityIdId && (
                        <small className="p-error">
                          Stone quality is required.
                        </small>
                      )}
                    </div>
                  </div>
            ...
            <div className="col-12 md:col-6 lg:col-6">
              <Fieldset className="p-0 m-0">
                <div className="col-12 md:col-12 lg:col-12 p-0">
                  <div className="flex">
                    <div className="col-12 md:col-4 lg:col-4">
                      <label htmlFor="WtEa" className="font-bold block my-2">
                        Wt Ea
                      </label>
                    </div>
                    <div className="col-12 md:col-8 lg:col-8">
                      <InputNumber
                        className="w-full"
                        value={state?.WtEa}
                        onValueChange={(e) => handleWtEaChange(e)}
                        id="WtEa"
                        name="WtEa"
                        minFractionDigits={3}
                        maxFractionDigits={3}
                ...
                <div className="col-12 md:col-12 lg:col-12 p-0">
                  <div className="flex">
                    <div className="col-12 md:col-4 lg:col-4">
                      <label htmlFor="WtEaRd" className="font-bold block my-2">
                        Wt Ea/Rd
                      </label>
                    </div>
                    <div className="col-12 md:col-8 lg:col-8">
                ...
                <div className="col-12 md:col-12 lg:col-12 p-0">
                  <div className="flex">
                    ...
                    <div className="col-12 md:col-8 lg:col-8">
                      <InputNumber
                        value={state?.Pct}
                        onValueChange={(e) => handlePctChange(e)}
                        id="Pct"
                        name="Pct"
                        mode="currency"
                        currency="USD"
                  ...
                  <div className="flex">
                    ...
                    <div className="col-12 md:col-8 lg:col-8">
                      <InputNumber
                        ...
                        maxFractionDigits={2}
                        className={classNames(
                          { "p-invalid": submitted && !state?.LanEa },
                          "w-full"
                        )}
                        min={0}
                        disabled={isReadOnly}
                      />
                      {submitted && !state?.LanEa && (
                        <small className="p-error">$/Lan Ea is required.</small>
                      )}
                    </div>
                  </div>
                </div>
      ...
      >
        ...
        <div className="col-12 md:col-12 lg:col-12">
          <Checkbox
            inputId="AllStoneFields"
            name="AllStoneFields"
            value="AllStoneFields"
            onChange={onAllStoneFieldChange}
            checked={allStoneFieldChecked}
          />
          <label htmlFor="AllStoneFields" className="ml-2">
            All
          </label>
        </div>
        <div className="col-12 md:col-12 lg:col-12">
              ...
              return (
                <div key={item} className="flex align-items-center w-8rem">
                  <Checkbox
                    ...
                    checked={selectedStones.some((x) => x === item)}
                  />
      ...
      >
        ...
        <div className="col-12">
          <FileUpload
            name="UploadStoneDataCSV"
            onUpload={(e) => onStoneDataCSVFileUpload(e.files)}
            url="/"
          />
          <small className="p-error">
            Note: Please import maximum 500 stones at a time.
          </small>
        </div>
      </Dialog>
    </div>
