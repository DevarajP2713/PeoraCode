...
const PeoraContextProvider = (props: any) => {
  ...
  const [refactorProductCategoryData, setRefactorProductCategoryData] =
    useState([]);
  const [refactorProductSubCategoryData, setRefactorProductSubCategoryData] =
    useState([]);
  ...
  const [refactorAllPricingSchemeData, setRefactorAllPricingSchemeData] =
    useState([]);
  ...
  // Tech changes
  const [masterPhotos, setMasterPhotos] = useState<any[]>([]);
  ...
  const _getAllPhotos = async () => {
    const _allPhotos = await getAllLibraryDatas();
    setMasterPhotos([..._allPhotos]);
  };
  ...
  const getAllInventory = async () => {
    ...
    const preparedAllInventoryData = inventory?.map((ele) => {
      return {
        ...
        OfficeQty: ele?.OfficeQty,
        TotalCost: ele?.TotalCost,
        Photos: ele?.Photos,
        VendorRef1: ele?.VendorRef1,
        RingSize: ele?.RingSize,
        MetalType: ele.MetalTypeId ? ele.MetalType.MetalTypeName : "",
        StoneDetails: ele.StoneDetails ? JSON.parse(ele.StoneDetails) : [],
      };
    });
  ...
  const getAllPricingScheme = async () => {
    const allPricingScheme = await getMasterListsData(
      LISTS.PRICING_SCHEME.NAME
    );
  ...
  const getAllBoxType = async () => {
    ...
    const allBoxType = await getMasterListsData(
      LISTS.BOX_TYPE.NAME,
      selectProperties,
      expandProperties
    );
  ...
  const getAllInsertType = async () => {
    const selectProperties = [
      "*,BaseMetal/Title,BaseMetal/Id,Category/CatName,Category/Id",
    ];
    const expandProperties = ["BaseMetal,Category"];

    const allInsertType = await getMasterListsData(
      LISTS.INSERT_TYPE.NAME,
      selectProperties,
      expandProperties
    );
  ...
  const getAllChainType = async () => {
    const selectProperties = [
      "*,Metal/MetalTypeName,Metal/Id,Category/CatName,Category/Id",
    ];
    const expandProperties = ["Metal,Category"];

    const allChainType = await getMasterListsData(
      LISTS.CHAIN_TYPE.NAME,
      selectProperties,
      expandProperties
    );
  ...
  const getAllBackingType = async () => {
    ...
    const allBackingType = await getMasterListsData(
      LISTS.BACKING_TYPE.NAME,
      selectProperties,
      expandProperties
    );
  ...
  const getRecoverableCost = async () => {
    ...
    const allRecoverableCost = await getMasterListsData(
      LISTS.RECOVERABLE_COST.NAME,
      selectProperties,
      expandProperties
    );
  ...
  const getAllSellingPrice = async () => {
    ...
    const allSellingPrice = await getMasterListsData(
      LISTS.SELLING_PRICE.NAME,
      selectProperties,
      expandProperties
    );
    ...
    const preparedAllSellingPrice = allSellingPrice?.map((ele) => {
      return {
        Id: ele?.Id,
        ChanelName: ele?.ChanelName,
        MultiplyBy: ele?.MultiplyBy,
        DivideBy: ele?.DivideBy,
        Plus: ele?.Plus,
        BaseMetal: ele?.BaseMetal,
      };
    });
  ...
  useEffect(() => {
    (async () => {
      await getAllProductCategoryData();
      await getAllProductSubCategoryData();
      await getAllProductMetalTypeData();
      await getAllStoneTypeData();
      await getAllStoneShapeData();
      await getAllStoneSizeData();
      await getAllStoneCutTypeData();
      await getAllStoneQualityData();
      await getAllVendorData();
      await getAllStones();
      await getPurchaseOrder();
      await getLengthData();
      await getWidthData();
      await getAllPricingScheme();
      await getAllCustomer();
      await getAllSalesOrderData();
      await getAllBaseMetal();
      await getAllBaseStone();
      await getAllBoxType();
      await getAllInsertType();
      await getAllChainType();
      await getAllBackingType();
      await getAllParentage();
      await getAllInventory();
      await getHeaderMenuData();
      await getRecoverableCost();
      await getAllSellingPrice();
