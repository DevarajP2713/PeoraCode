...
export const checkUserHasPermissionForManageWeb = async () => {
  try {
    ...
    console.log(
      "checkUserHasPermissionForManageWeb...error...",
      JSON.stringify(ex)
    );
...
// Tech changes start
export const getPagedValues = async (
  listName: string,
  data: any,
  Filtercondition: any,
  arrDatas: any[]
  ...
  return await sp.web.lists
    ...
    .then(async (data: any) => {
      ...
      if (data.NextHref && arrDatas.length < 5000) {
        await getPagedValues(
          listName,
          data.NextHref,
          Filtercondition,
          arrDatas
...
export const getAllLibraryPhotos = async (
  listName: string,
  Filtercondition: any
) => {
  const arrDatas: any[] = [];
...
export const getAllSPListItems = async (
  listName: string,
...
export const getSPListItems = async (
  listName: string,
  selectProperties: string[],
  expandProperties: string[],
  filterQuery: string,
...
export const addItemToSPList = async (listName: string, item: any) => {
  try {
    const sp: SPFI = getSP();
    const insertedItem = await sp.web.lists
      .getByTitle(listName)
      .items.add(item);
    return insertedItem;
  } catch (ex) {
...
export const deleteItemFromSPList = async (
  listName: string,
  itemId: number
) => {
  try {
...
export const deleteSPListItemAttachmentFile = async (
  listName: string,
  itemId: number,
  fileName: string
) => {
  ...
  return sp.web.lists
    ...
    .catch((err) => {
      console.log(
        "Service...deleteSPListItemAttachmentFile...error...",
        JSON.stringify(err)
      );
      return err;
    });
...
export const updateSingleFieldValueToSPList = async (
  listName: string,
  itemId: number,
  ...
  try {
    ...
    return sp.web.lists
      ...
      .catch((err) => {
        console.log(
          "error...updateSingleFieldValueToSPList...",
          JSON.stringify(err)
        );
        return err;
      });
    ...
    console.log(
      "updateSingleFieldValueToSPList...error...",
      JSON.stringify(ex)
    );
    throw ex.message.value;
  }
...
export const checkCurrentUserGroups = async () => {
  ...
  let filterAdminUserGroup = groups.filter(
    ({ LoginName }) => LoginName === ADMIN_USER_GROUP
  );
...
export const updateInventoryProductMultipleFields = async (
  listName,
  itemId,
  newMtWtFoB,
  newMtWtFeed,
...
export const updateInventoryOrderMultipleFields = async (
  listName,
  itemId,
  officeQty,
  onOrder
